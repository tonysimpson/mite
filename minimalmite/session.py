...
class Response:
	...
	def status_code(self):
		return self._creq.getinfo(pycurl.RESPONSE_CODE)
...
class session:
	...
	async def request(self, url, method=None, params=None, data=None, headers=None, cookies=None, allow_redirects=True):
		creq = pycurl.Curl()
		creq.setopt(pycurl.URL, url)
		creq.setopt(pycurl.SHARE, self._shared_handle)
		creq.setopt(pycurl.FOLLOWLOCATION, allow_redirects)
		if method is not None:	
			creq.setopt(pycurl.CUSTOMREQUEST, method)
		creq.body = BytesIO()
		creq.setopt(pycurl.WRITEFUNCTION, creq.body.write)
		if self._metrics_callback is not None:
			metrics = {'time': time.time(), 'url': url}
		await self._controller.perform(self._multi, creq)
		if self._metrics_callback is not None:
			metrics.update(get_metrics(creq))
			self._metrics_callback(metrics)
		return Response(creq)
