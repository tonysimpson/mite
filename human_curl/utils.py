...
from urllib.parse import urlunparse, urlparse, parse_qs, quote, unquote, urlencode
...
def make_curl_post_files(data):
    ...
    def checker(name):
        if file_exists(name):
            return (pycurl.FORM_FILE, name)
        else:
            raise RuntimeError("File %s doesn't exist" % name)
...
def parse_dict_header(value):
    ...
    for item in parse_http_list(value):
        ...
        if value[:1] == value[-1:] == '"':
            value = unquote(value[1:-1]) # strip " and unquote
        result[name] = value
    return result
